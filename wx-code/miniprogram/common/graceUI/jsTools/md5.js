...
function rstr_hmac_md5(r, t) {
    var d = rstr2binl(r);
    16 < d.length && (d = binl_md5(d, 8 * r.length));
    for (var n = Array(16), _ = Array(16), m = 0; m < 16; m++) n[m] = 909522486 ^ d[m], 
    _[m] = 1549556828 ^ d[m];
...
function rstr2hex(r) {
    for (var t, d = hexcase ? "0123456789ABCDEF" : "0123456789abcdef", n = "", _ = 0; _ < r.length; _++) t = r.charCodeAt(_), 
    n += d.charAt(t >>> 4 & 15) + d.charAt(15 & t);
    return n;
}

function rstr2b64(r) {
    for (var t = "", d = r.length, n = 0; n < d; n += 3) for (var _ = r.charCodeAt(n) << 16 | (n + 1 < d ? r.charCodeAt(n + 1) << 8 : 0) | (n + 2 < d ? r.charCodeAt(n + 2) : 0), m = 0; m < 4; m++) 8 * n + 6 * m > 8 * r.length ? t += b64pad : t += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(_ >>> 6 * (3 - m) & 63);
    return t;
}
...
function rstr2any(r, t) {
    ...
    for (n = 0; n < a; n++) {
        for (f = Array(), d = m = 0; d < e.length; d++) m = (m << 16) + e[d], m -= (_ = Math.floor(m / h)) * h, 
        (0 < f.length || 0 < _) && (f[f.length] = _);
        i[n] = m, e = f;
    }
    var o = "";
    for (d = i.length - 1; 0 <= d; d--) o += t.charAt(i[d]);
    return o;
}

function str2rstr_utf8(r) {
    for (var t, d, n = "", _ = -1; ++_ < r.length; ) t = r.charCodeAt(_), d = _ + 1 < r.length ? r.charCodeAt(_ + 1) : 0, 
    55296 <= t && t <= 56319 && 56320 <= d && d <= 57343 && (t = 65536 + ((1023 & t) << 10) + (1023 & d), 
    _++), t <= 127 ? n += String.fromCharCode(t) : t <= 2047 ? n += String.fromCharCode(192 | t >>> 6 & 31, 128 | 63 & t) : t <= 65535 ? n += String.fromCharCode(224 | t >>> 12 & 15, 128 | t >>> 6 & 63, 128 | 63 & t) : t <= 2097151 && (n += String.fromCharCode(240 | t >>> 18 & 7, 128 | t >>> 12 & 63, 128 | t >>> 6 & 63, 128 | 63 & t));
    return n;
}
...
function safe_add(r, t) {
    ...
    return (r >> 16) + (t >> 16) + (d >> 16) << 16 | 65535 & d;
}
